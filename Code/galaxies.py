...
def get_mthreshHOD_lsst(zs_bins):
    fname = "data/mthreshHOD_lsst.txt"
...
def n_arcmin2_LSST_Schmittfull(z):
    ...
    data = np.loadtxt(fname="data/n_schmittfull.txt",dtype=dt)
...
def N_tot_angular(n_arcmin2,z_min,z_max,surveyarea_sqdeg): #n_arcmin2 is an array over z
    ...
    N = np.trapz(n_arcmin2[zmin_id:zmax_id],z_sampling[zmin_id:zmax_id])
...
def match_ngal_mthreshHOD():
    ...
    fig.tight_layout()
    plt.show()    
    #fig.savefig('data/bias_lsst_mthreshHODmatching.pdf')    
